...
<body>
    ...
    <div class="d-flex justify-content-center flex-column align-items-center mt-5">
        ...
        <table class="table w-75">
            ...
            <tbody>
                ...
                if(isset($_GET['search'])){
                    $dataMahasiswa = mysqli_query(mysqli_connect("localhost", "root", "", "mahasiswa", 3308), "select * from `data` where CONCAT(NIM, Nama, Alamat, Fakultas) like '%" .$_GET['search']. "%' limit $posisi, $batas");
                } else {
                    $dataMahasiswa = mysqli_query(mysqli_connect("localhost", "root", "", "mahasiswa", 3308), "select * from `data` limit $posisi, $batas");
                }
                    ...
                    <div class="modal-dialog modal-dialog-centered">
                        <div class="modal-content">
                            ...
                            <div class="modal-footer">
                                ...
                                <a type="button" class="btn btn-danger" href="backend.php?nim=<?= $nim ?>&delete">Hapus</a>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        <?php
            ...
            if(isset($_GET['search'])){
                $search = $_GET['search'];
                $sqlPage = mysqli_query(mysqli_connect("localhost", "root", "", "mahasiswa", 3308), "select * from `data` where CONCAT(NIM, Nama, Alamat, Fakultas) like '%" .$_GET['search']. "%'");
                $jumlahData = mysqli_num_rows($sqlPage);
                $jumlahHalaman = ceil($jumlahData/$batas);
            } else {
                $sqlPage = mysqli_query(mysqli_connect("localhost", "root", "", "mahasiswa", 3308), "select * from `data`");
                $jumlahData = mysqli_num_rows($sqlPage);
                $jumlahHalaman = ceil($jumlahData/$batas);
            }
        ?>
