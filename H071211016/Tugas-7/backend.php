    ...
    public function login(){
        if (isset($_POST['login'])){
            ...
            if(!empty($email) && !empty($passwordInput)){
                $sql = mysqli_query(mysqli_connect("localhost", "root", "", "mahasiswa", 3308), "select std_id, Nama, Email, Password from account where Email = '$email'");
        
                $row = mysqli_fetch_array($sql);

                $password = password_verify($passwordInput, $row['Password']);
