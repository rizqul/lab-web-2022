<?php
function requestConnection() { // Membuka koneksi server ke database
    if (session_status() == PHP_SESSION_NONE) {
        session_start();
    }
    ob_start();

    global $conn; // Nanti bakal di panggil pake variabel $GLOBALS['conn'] di fungsi lain
...
function setButtons($conn) { // Membaca seluruh tombol dalam dokumen dan menyetel Fungsinya
    ...
    if (isset($_POST['save'])) {
        $project_name = setDataTo('project_name');
        $head_staff = setDataTo('head_staff');
        $research_code = setDataTo('research_code');
        $status = setDataTo('status');
